...
to GO
  ...
  ask turtles [
    ...
    ;fd 0.01
    ;create-link-to one-of other turtles in-radius 10
    ...
    ask out-link-neighbors [
      let packetloss 0.01
      if random-float 1 > packetloss [
        let setnoise 0.2
        set consensus-x consensus-x + ratio * ( position-x + (random-float setnoise * 2 - setnoise))
        set consensus-y consensus-y + ratio * ( position-y + (random-float setnoise * 2 - setnoise))
      ]
    ]
